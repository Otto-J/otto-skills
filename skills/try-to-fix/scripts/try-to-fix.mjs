...
import { ensureDir, runCommand, sha256File, ZIP_LIMITS } from './bundle-utils.mjs'
import { loadErrorMessageMappings, mapErrorEvidence } from './error-message-mapper.mjs'
...
const DEVICE_ID_RE =
  /(?:\*\*(?:Device\s*ID|device_id):\*\*|(?:Device\s*ID|device_id):)\s*`?([^`\n]+)`?/i
...
const TOKEN_REDACTION_RE =
  /\b(?:ghp|gho|ghu|ghs|ghr|github_pat|sk|xox[baprs]|glpat)-[A-Za-z0-9_=-]{10,}\b/g
const KEY_VALUE_SECRET_REDACTION_RE =
  /\b([A-Za-z0-9_.-]*(?:token|secret|password|passwd|pwd|apikey|api_key|authorization)[A-Za-z0-9_.-]*\s*[:=]\s*)([^\s'",;]+)/gi
...
const CORRELATION_FIELD_REDACTION_RE =
  /\b((?:sessionId|session_id|sessionKey|session_key|promptId|prompt_id|turnId|turn_id|traceId|trace_id|clientMessageId|client_message_id)\s*[:=]\s*["']?)([A-Za-z0-9._:-]+)/gi
const BARE_CORRELATION_RE =
  /\b(?:desktop-local-subagent|session|prompt|turn|trace|client-message|message)[-_:][A-Za-z0-9._:-]{6,}\b/gi
const UUID_REDACTION_RE =
  /\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b/gi
const CONVERSATIONAL_FIELD_REDACTION_RE =
  /\b((?:systemPrompt|userPrompt|promptText|assistantResponse)\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\n,;}]+)/gi
const SENTRY_PRIVATE_QUERY_RE = /\b((?:device_id|user\.id|user):)([^\s]+)/gi
const SIGNED_URL_SECRET_REDACTION_RE =
  /([?&](?:X-Goog-[A-Za-z-]*|X-Amz-[A-Za-z-]*|GoogleAccessId|OSSAccessKeyId|AccessKeyId|Signature|Expires|x-oss-[A-Za-z-]*|token|credential)=)[^&#\s)]+/gi
...
async function checkSentryDependencies() {
  ...
  if (!sentryToken) {
    throw new Error(
      'Sentry 鉴权失败：未找到 SENTRY_AUTH_TOKEN 或 ~/.sentryclirc token。候选证据不能降级为空。 '
    )
  }
  return sentryToken
}
...
async function fetchIssueByNumber(repo, issueNumber) {
  ...
  issue.comments = JSON.parse(commentsJson)
    .flat()
    .map((comment) => ({
...
async function downloadAttachment(attachment, outputDir, index, options = {}) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const curlArgs = [
      ...
      '300',
      '--max-filesize',
      String(ZIP_LIMITS.compressedBytes),
      '-o',
      partial,
      attachment.url
    ]
...
function redactTrustedAttachmentUrls(value) {
  const markdownRedacted = String(value || '').replace(MARKDOWN_LINK_RE, (match, label, url) =>
    isTrustedAttachmentUrl(url.replace(/&amp;/g, '&'))
      ? `[${label}]([redacted-attachment-url])`
      : match
  )
  return markdownRedacted.replace(/https:\/\/[^\s<>()]+/gi, (url) =>
    isTrustedAttachmentUrl(url.replace(/&amp;/g, '&')) ? '[redacted-attachment-url]' : url
  )
}

function sanitizeIssueEvidenceText(value) {
  return redactSensitiveText(redactTrustedAttachmentUrls(value)).trim()
}
...
export function normalizeIssueComments(comments) {
  ...
  return comments.map((comment) => ({
    ...
    createdAt: comment?.createdAt || '',
    url: comment?.url || '',
    body: sanitizeIssueEvidenceText(comment?.body || '')
  }))
}

export function buildIssueEvidence(issue, repo = DEFAULT_REPO) {
  return {
    number: issue.number,
    url: issue.url || `https://github.com/${repo}/issues/${issue.number}`,
    title: sanitizeIssueEvidenceText(issue.title),
    body: sanitizeIssueEvidenceText(issue.body),
...
export function resolveSentryTarget({ feedback, logAnalysis, project, projectOverride = false }) {
  ...
  const versions = [...new Set([envInfo.appVersion, feedback?.version])]
    .filter((value) => typeof value === 'string' && value.trim())
    .map((value) => value.trim())
  const version = firstString(...versions)
  const releases = versions.flatMap((candidate) =>
    mobile ? [`cola-mobile@${candidate}`] : [`cola-server@${candidate}`, `cola@${candidate}`]
  )
  return {
    project: projectOverride ? project : mobile ? DEFAULT_MOBILE_PROJECT : DEFAULT_DESKTOP_PROJECT,
    environment: mobile
      ? firstString(envInfo.buildChannel, DEFAULT_SENTRY_ENVIRONMENT)
...
export function buildSentryCandidateQueries({
  environment,
  deviceId,
  userId,
  releases,
  ...
  if (deviceId && String(deviceId).toLowerCase() !== 'unknown') {
    ...
    if (environment) {
      add(
        'device-environment',
        `device_id:${quoteSentryValue(deviceId)} environment:${quoteSentryValue(environment)}`,
        ['device_id']
      )
    }
  }
  ...
  const tokens = phrases
    .flatMap((phrase) => phrase.match(/[A-Za-z][A-Za-z0-9_.-]{4,}/g) || [])
...
async function runSentryCli(args) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      lastError = error
      const detail = String(error.stderr || error.stdout || error.message || error)
      if (!/HTTP2 framing|timed out|timeout|connection|network/i.test(detail) || attempt === 3)
        break
      ...
      (response) => {
        ...
        response.on('end', () => {
          ...
          if (status < 200 || status >= 300) {
            reject(
              new Error(
                `Sentry API HTTP ${status}: ${data?.detail || data?.message || 'unknown error'}`
              )
            )
            return
          }
...
function safeBreadcrumbEvidence(detail) {
  ...
  return values.slice(-15).map((value) => {
    ...
    const conversational =
      /prompt|conversation|chat\.message|llm\.input|user\.input|assistant\.response/i.test(
...
async function collectSentryCandidates({
  ...
  if (queryRoutes.length === 0) {
    throw new Error(
      '没有足够信息构造 Sentry 候选查询；至少需要环境、版本、身份线索或错误短语之一。'
    )
  }
  ...
  const queryResults = await mapWithConcurrency(queryRoutes, 3, async (route) => {
    try {
      return {
        route,
        rows: await querySentryIssues({ org, project, query: route.query }),
        error: ''
      }
    } catch (error) {
      return {
  ...
  for (const result of queryResults) {
    for (const row of result.rows) {
      ...
      if (!current.sourceReasons.includes(result.route.reason))
        current.sourceReasons.push(result.route.reason)
  ...
  const ranked = [...candidates.values()].toSorted((left, right) => {
    return (
      mappingSourcePriority(right) - mappingSourcePriority(left) ||
  ...
  await mapWithConcurrency(sampled, 4, async (candidate, candidateIndex) => {
    try {
      ...
      candidate.eventSamples = selectedEvents.map((event, index) =>
        sanitizeEvent(event, { deviceId, userId }, index === 0 ? detail : null)
      )
    } catch (error) {
...
function mergeLogAnalyses(analyses, feedback) {
  if (analyses.length === 0) {
    return {
      ...
      errorBlocks: [],
      rawErrorLog: '',
      userPath: '未发现可分析的 zip 日志附件。',
      errorPhrases: [],
      errorTime: null,
      correlations: [],
      supplementalEvidence: [],
      status: '未发现可分析的 zip 日志附件。'
    }
  }
  const inventory = {}
  ...
  const primary =
    withTimes.toSorted((left, right) => {
      if (Number.isNaN(target)) return 0
      return (
        Math.abs(new Date(left.errorTime).getTime() - target) -
  ...
  return {
    ...
    errorBlocks: analyses.flatMap((analysis, index) =>
      (analysis.errorBlocks || []).map((block) => ({
        bundle: analysis.attachmentIndex || index + 1,
        ...block
      }))
    ),
    rawErrorLog: analyses
      .map((analysis) => analysis.rawErrorLog)
      .filter(Boolean)
      .join('\n\n'),
    ...
    supplementalEvidence: analyses.flatMap((analysis, index) =>
      (analysis.supplementalEvidence || []).map((item) => ({
        bundle: analysis.attachmentIndex || index + 1,
        ...item
      }))
    ),
    status: analyses
      .map((analysis, index) => `附件 ${analysis.attachmentIndex || index + 1}: ${analysis.status}`)
      .join(' ')
  }
}
...
export function assessEvidenceScope({
  feedback = {},
  downloads = [],
  logAnalysis = {},
  sentry = {}
}) {
  const hasValidatedAttachment = downloads.some(
    (download) => !download.failed && !download.duplicateOf
  )
  const hasSelectedLog = (logAnalysis.selectedLogs || []).length > 0
  const hasErrorTime = Boolean(logAnalysis.errorTime)
  const hasErrorPhrase = (logAnalysis.errorPhrases || []).length > 0
  const hasIdentity = Boolean(feedback.deviceId || feedback.device || feedback.userId)
  const hasRelease = (sentry.releases || []).length > 0
  const anchorReasons = [
    hasValidatedAttachment && 'validated-attachment',
    hasSelectedLog && 'selected-log',
    hasErrorTime && 'error-time',
    hasErrorPhrase && 'error-phrase',
    hasIdentity && 'identity',
    hasRelease && 'release'
  ].filter(Boolean)
  const anchorQuality =
    hasSelectedLog && (hasErrorTime || hasErrorPhrase || hasIdentity || hasRelease)
      ? 'strong'
      : anchorReasons.length > 0
        ? 'partial'
        : 'none'
  return {
    mode: anchorQuality === 'none' ? 'issue-and-code-only' : 'runtime-backed',
    anchorQuality,
    anchorReasons
  }
}
...
function buildEvidence({
  ...
  return {
    ...
    feedback: {
      ...
      runtimeVersion: redactSensitiveText(logAnalysis.envInfo?.appVersion || ''),
      runtimeBuildChannel: redactSensitiveText(logAnalysis.envInfo?.buildChannel || '')
    },
    evidenceScope,
    attachments: {
      ...
      expectedCountMatches:
        feedback.expectedFileCount === null
          ? null
    ...
    sentry: {
      ...sentry,
      anchorQuality: evidenceScope.anchorQuality,
      anchorReasons: evidenceScope.anchorReasons
    },
