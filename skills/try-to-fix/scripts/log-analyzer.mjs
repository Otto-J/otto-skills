...
const DIAGNOSTIC_PATTERNS = [
  { pattern: /messages\.\d+\.content\.\d+\.image\.source\.base64\.media_type|Input should be 'image\/jpeg'/i, score: 110 },
  { pattern: /not connected to cola server|server connection lost|econnrefused|websocket error|enotfound|err_network|err_timed_out/i, score: 100 },
...
function logKindForPlatform(platform) {
  ...
  if (normalized === "mac" || normalized === "macos" || normalized === "windows" || normalized === "win") return "desktop";
  return "";
}
...
function chooseLogFiles(logs, createdAt, platform) {
  ...
  if (targetDate) {
    const exact = orderByPreferredLogKind(
      logs.filter((log) => log.dateKey === targetDate), preferredKind,
    );
    if (exact.length > 0) {
    ...
    if (closestDate) {
      return orderByPreferredLogKind(
        logs.filter((log) => log.dateKey === closestDate), preferredKind,
...
function compactLogLine(line) {
  const cleaned = line
    ...
    .replace(/("(?:systemPrompt|userPrompt|prompt|content)"\s*:\s*)"[^"]*"/gi, "$1\"<redacted content>\"")
...
function collectErrorBlocks(lines, maxBlocks = DEFAULT_MAX_BLOCKS, options = {}) {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    if (!ERROR_PATTERNS[0].test(lines[index])) continue;
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    if (isNoisyContextLine(lines[index])) continue;
    if (isMobileStackFrameLine(lines[index])) continue;
    if (!ERROR_PATTERNS.slice(1).some((pattern) => pattern.test(lines[index]))) continue;
...
function collectUserPath(lines, errorIndex) {
  const searchWindow = errorIndex === null || errorIndex === undefined
    ? lines.slice(-180)
...
function buildNextStep(blocks) {
  ...
  if (joined.includes("econnrefused") || joined.includes("connection lost") || joined.includes("not connected to cola server")) {
...
function classifyBundleFiles(root) {
  ...
  return {
    ...
    summary: Object.fromEntries(
      Object.entries(groups).map(([kind, files]) => [kind, files.length]),
    ),
  };
}
...
function collectCorrelations(lines) {
  const pattern = /\b(sessionId|session_id|sessionKey|session_key|promptId|prompt_id|turnId|turn_id|traceId|trace_id|clientMessageId|client_message_id)\s*[:=]\s*["']?([A-Za-z0-9._:-]{4,})/gi;
  const results = [];
  const seen = new Set();
...
function supplementalTimestamp(lines) {
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    const timestamps = [...lines[index].matchAll(/"(?:ts|timestamp|createdAt|updatedAt|time)"\s*:\s*"([^"]+)"/g)]
...
function collectSupplementalEvidence(root, groups, correlations, feedbackTime) {
  ...
  for (const candidate of candidates) {
    ...
    const matched = correlations.filter((item) => scanned.lines.some((line) => line.includes(item.raw)));
    ...
    const inTimeWindow = !Number.isNaN(feedbackMs)
      && !Number.isNaN(evidenceMs)
      && Math.abs(feedbackMs - evidenceMs) <= 2 * 60 * 60 * 1000;
    const hasFailureSignal = signals.some((signal) => ["error", "timeout", "cancelled"].includes(signal));
    if (matched.length === 0 && !hasFailureSignal && !inTimeWindow) continue;

    const selectedBy = matched.length > 0
      ? "correlation"
      : inTimeWindow
        ? "time-window"
        : "failure-signal";
    ...
    evidence.push({
      ...
      selectedBy,
      matchedCorrelations: matched.map(({ kind, fingerprint: value }) => ({ kind, fingerprint: value })),
      signals,
      completionHints: {
...
function closestErrorBlock(blocks, createdAt) {
  ...
  return blocks.toSorted((left, right) => {
    ...
    const leftDistance = Number.isNaN(leftTime) ? Number.POSITIVE_INFINITY : Math.abs(leftTime - target);
    const rightDistance = Number.isNaN(rightTime) ? Number.POSITIVE_INFINITY : Math.abs(rightTime - target);
...
export async function analyzeFeedbackBundle({ zipPath, workspaceDir, feedback, maxBlocks = DEFAULT_MAX_BLOCKS }) {
  if (!zipPath) {
  ...
  if (selectedLogs.length === 0) {
    return {
      ...emptyAnalysis("当前附件里没有找到 logs/cola-YYYY-MM-DD.log 或 logs/cola-mobile-YYYY-MM-DD.log，先补日志再分析。"),
      root,
      envInfo,
  ...
  const primaryData = selectedData.find(
    (item) => item.selected.relPath === primaryError?.logRelativePath,
  ) || selectedData[0];
  ...
  return {
    ...
    inventory: inventory.summary,
    correlations: privateCorrelations.map(({ kind, fingerprint: value }) => ({ kind, fingerprint: value })),
    supplementalEvidence,
    errorBlocks,
    rawErrorLog,
