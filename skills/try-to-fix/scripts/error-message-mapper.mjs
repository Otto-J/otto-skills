...
function normalizeRule(rule) {
  ...
  const exact = Array.isArray(rule.exact) ? rule.exact.map(normalizeMatcher).filter(Boolean) : []
...
export async function loadErrorMessageMappings(options = {}) {
  ...
  const candidates = options.configPath
    ...
    : repoRoots.flatMap((repoRoot) =>
        DEFAULT_CONFIG_PATHS.map((candidate) => ({
  ...
  for (const candidate of candidates) {
    try {
      ...
      return {
        available: true,
        configPath: path
          .relative(candidate.repoRoot, candidate.configPath)
          .split(path.sep)
          .join('/'),
